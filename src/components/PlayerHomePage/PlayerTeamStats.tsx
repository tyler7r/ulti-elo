...
// import {
//   // Add PlayerAwardType definition (adjust based on your actual structure)
//   PlayerAwardType,
//   PlayerTeamsType,
//   PlayerTeamType,
//   SeasonType,
// } from "@/lib/types"; // Adjust path
// import { EmojiEventsSharp, WhatshotSharp } from "@mui/icons-material";
...
//   FormControl,
//   IconButton,
//   InputLabel, // Added for award list item
//   ListItemIcon, // Added for award text
//   MenuItem,
//   Paper,
//   Select,
//   SelectChangeEvent,
...
// import { getAwardDetails } from "../TeamHomePage/StatsTab/PlayerAwardsDisplay";
...
//   currentTeamData: PlayerTeamsType; // Current live data for the player on this team
//   ptId: string; // The specific player_teams ID for this player/team combo
//   teamId: string;
...
//   const [seasons, setSeasons] = useState<SeasonType[]>([]); // List of seasons for this team
//   const [selectedSeasonId, setSelectedSeasonId] = useState<string>("current"); // 'current' or season.id
//   const [displayStats, setDisplayStats] = useState<PlayerTeamType | null>(null); // Stats to display (current or past)
//   const [eloHistory, setEloHistory] = useState<EloHistoryEntry[]>([]);
//   const [seasonAwards, setSeasonAwards] = useState<PlayerAwardType[]>([]); // <-- New state for awards
//   const [loadingSeasons, setLoadingSeasons] = useState(true);
//   const [loadingStats, setLoadingStats] = useState(false); // Loading for selected season's stats
//   const [loadingEloHistory, setLoadingEloHistory] = useState(false);
//   const [loadingAwards, setLoadingAwards] = useState(false); // <-- New loading state for awards
//   const [error, setError] = useState<string | null>(null);
//   const [activeSeasonIdCache, setActiveSeasonIdCache] = useState<string | null>(
//     null
//   ); // Cache active season ID

//   const currentPlayerData = currentTeamData.player; // Convenience alias
...
//         // If not in the list and not cached, fetch it
//         const activeId = await fetchActiveSeasonId();
//         if (activeId) {
//           // Potentially fetch the full active season details if needed elsewhere,
//           // but for now, we just needed the ID check.
//           // Or assume it *should* have been in the initial fetch if truly active.
//         }
//       } else if (!activeSeason && activeSeasonIdCache) {
//         // Use cached ID if available, implies active season might not be in main list (edge case?)
//       }

//       if (activeSeason || activeSeasonIdCache) {
//         setSelectedSeasonId("current");
//       } else if (fetchedSeasons.length > 0) {
//         setSelectedSeasonId(fetchedSeasons[0].id); // Default to most recent completed
//       } else {
//         setSelectedSeasonId("current"); // Fallback
//       }
//     } catch (err) {
...
//     }
//   }, [teamId, fetchActiveSeasonId, activeSeasonIdCache]);
...
//   // --- Fetch Player Awards for Selected Season --- <--- NEW FUNCTION
...
//       const targetSeasonId = await getTargetSeasonId();
//       if (!targetSeasonId) {
//         // Optionally set an award-specific error or just show "no awards" later
...
//       // Clear global error *only if* this fetch was the source of a previous error
//       // if (error && error.includes("awards")) setError(null);
//     } catch (error) {
...
//     // Reset dependent states when season changes
//     setEloHistory([]);
//     setSeasonAwards([]); // <-- Reset awards on season change
//     setError(null); // Clear errors on season change
//     // Stats will be re-fetched by their own useEffect trigger
//   };
...
//       ? `Current Season (S${
//           seasons.find((s) => s.active)?.season_no ??
//           (activeSeasonIdCache
//             ? seasons.find((s) => s.id === activeSeasonIdCache)?.season_no
//             : "N/A") // Handle cached ID display
//         })`
//       : `Season ${
//           seasons.find((s) => s.id === selectedSeasonId)?.season_no ?? ""
//         }`;
//   const selectedSeasonNumber = seasons.find(
...
//                 onClick={() =>
//                   void router.push(`/team/${currentTeamData.team.id}`)
//                 }
//               />
//             ) : (
...
//         >
//           {selectedSeasonId === "current"
//             ? "No current stats found" // More direct message
...
//             {/* ... (Season Selector remains largely the same, check value/labels) ... */}
//             <InputLabel id={`season-select-label-${ptId}`}>Season</InputLabel>
//             <Select
//               labelId={`season-select-label-${ptId}`}
//               id={`season-select-${ptId}`}
//               value={selectedSeasonId} // Value comes from state
//               label="Season"
...
//                 loadingEloHistory ||
//                 loadingAwards
//               } // Disable while any dependent data loads
//             >
//               {/* Always render the "Current Season" option */}
...
//                     seasons.find((s) => s.active)?.season_no ??
//                       (activeSeasonIdCache
//                         ? seasons.find((s) => s.id === activeSeasonIdCache)
//                             ?.season_no
//                         : "...") // Show loading/pending state
//                   }
//                 </div>
//               </MenuItem>
//               {/* Render loading state or past seasons */}
//               {loadingSeasons ? (
...
//               ) : (
//                 seasons
//                   .filter((s) => !s.active) // Ensure only non-active are listed here
//                   .map((season) => (
...
//           {/* Rank, Peak ELO, Best Streak (based on selected season) */}
...
//                           <EmojiEventsSharp fontSize="small" color="warning" />
//                         </div>
//                       </TableCell>
//                       <TableCell>
...
//                             <WhatshotSharp color="warning" fontSize="small" />
//                           </div>
//                         )}
//                       </TableCell>
//                     </TableRow>
...
//                 Awards ({selectedSeasonLabel})
//               </Typography>
//               {loadingAwards ? (
//                 <Box sx={{ display: "flex", justifyContent: "center", py: 1 }}>
//                   <CircularProgress size={20} />
//                 </Box>
//               ) : error && error.includes("awards") ? ( // Check if the error is specific to awards
...
//                     const awd = getAwardDetails(award.award_type);
//                     return (
//                       <Box
//                         key={award.id}
//                         display={"flex"}
...
//                           {" "}
//                           {/* Adjust spacing */}
//                           <Tooltip title={awd.title} placement="left">
//                             {awd.icon}
//                             {/* TODO: Use different icons based on award_type if available */}
//                           </Tooltip>
//                         </ListItemIcon>
...
//                 </Box>
//               ) : (
//                 /* Alternative using Chips:
//                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
//                   {seasonAwards.map((award) => (
//                     <Tooltip key={award.id} title={award.award_description || award.award_name}>
//                       <Chip
...
//           <Box sx={{ px: 1 }}>
//             <Typography variant="body1" fontWeight={"bold"} gutterBottom>
//               Elo History - {selectedSeasonLabel}
//             </Typography>
//             {loadingEloHistory ? (
//               <Box sx={{ display: "flex", justifyContent: "center", py: 1 }}>
//                 <CircularProgress size={20} />
//               </Box>
//             ) : error && error.includes("Elo history") ? ( // Check if error is specific to Elo
//               <Typography variant="body2" color="error">
//                 {error}
//               </Typography>
//             ) : eloHistory.length > 0 ? (
...
//               </Typography>
//             )}
//             {/* Display general errors not caught by specific sections */}
//             {error &&
...
// /components/PlayerHomePage/PlayerTeamStats.tsx (Adjust path)
